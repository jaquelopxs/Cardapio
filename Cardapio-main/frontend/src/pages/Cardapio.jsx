...
export default function Cardapio({ adicionar }) {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/produtos")
      ...
      .catch(err => {
        console.warn("Backend não disponível, usando dados fictícios:", err);
        // Usar dados fictícios se backend não estiver disponível
        setProdutos(mockData.produtos);
        const cats = ["Todos", ...new Set(mockData.produtos.map(p => p.categoria).filter(Boolean))];
        setCategorias(cats);
      });
  }, []);
    ...
    <div style={{ minHeight: '100vh', background: '#f9fafb', padding: '32px 16px' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          <h1 style={{ fontSize: '36px', fontWeight: 'bold', color: '#5A0B1E', marginBottom: '8px' }}>Nosso Cardápio</h1>
          ...
          {categorias.map((cat) => (
            <button
              ...
              style={{
                ...
                boxShadow: categoriaSelecionada === cat ? '0 4px 12px rgba(90,11,30,0.3)' : '0 2px 6px rgba(0,0,0,0.1)',
              }}
            >
              {cat}
            </button>
